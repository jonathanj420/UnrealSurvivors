...
void AABSection::OnNPCSpawn()
{
	...
	auto KeyNPC = GetWorld()->SpawnActor<ATestCharacter>(GetActorLocation() + FVector::UpVector * 88.0f, FRotator::ZeroRotator);
	if (nullptr != KeyNPC)
	{
...
{
	auto TestCharacter = Cast<ATestCharacter>(DestroyedActor);
	ABCHECK(nullptr != TestCharacter);

	auto ABPlayerController = Cast<AABPlayerController>(TestCharacter->LastHitBy);
